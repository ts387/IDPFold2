...
class R3Diffuser:
    ...
    def forward_marginal(
        ...
        score_scaling_t = self.score_scaling(t)

        # Perturb only a subset of residues
        if diffuse_mask is not None:
            diffuse_mask = torch.as_tensor(diffuse_mask, device=x_t.device, dtype=x_t.dtype)[..., None]
    ...
    def score(self, x_t, x_0, t, scale=False, mask=None):
        t = inflate_array_like(t, x_t)
        if scale:
            x_t, x_0 = self.scale(x_t), self.scale(x_0)

        score = -(x_t - torch.exp(-0.5 * self.marginal_b_t(t)) * x_0) / self.conditional_var(t)
