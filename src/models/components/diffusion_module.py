...
if __name__ == '__main__':
    ...
    input_feature_dict = {
        'ref_pos': torch.ones(5, 64 * 14, 3),
        'label_pos': torch.ones(5, 64 * 14, 3),
        'atom_to_token_idx': torch.ones(64 * 14, dtype=torch.int64),
        'all_atom_pos_mask': torch.ones(5, 64 * 14, dtype=torch.bool),
        'ca_mask': torch.zeros(5, 64 * 14, 3),

        'residue_index': torch.ones(5, 64),
        'seq_mask': torch.ones(5, 64),
        'aatype': torch.ones(5, 64),
        'seq_emb': torch.ones(5, 64, 1280),

        'ref_space_uid': torch.ones(5, 64 * 14),
        'ref_atom_name_chars': torch.ones(5, 64 * 14, 4 * 64, dtype=torch.int64),
        'ref_element': torch.ones(5, 64 * 14, 128),
        'ref_charge': torch.ones(5, 64 * 14),
        'ref_mask': torch.ones(5, 64 * 14),
    }
