...
def parse_motif(pdb_path, contig_str, nsamples=1, make_tensor=False, motif_only=False, min_length=None,
                max_length=None):
...
def generate_indices_and_mask(contig: str, min_length: int, max_length: int) -> Tuple[int, List[int], np.ndarray]:
    ...
    while not success:
        ...
        if min_length is None and max_length is None:
            success = True
        elif (current_position - 1) >= min_length and (
                current_position - 1) <= max_length:  # cur_pos starts at 1 so after adding 3 its 4
            success = True
    output_string = output_string[:-1]
    ...
    if motif_only:
        ...
        for i in position:
            ...
            if atom_part == "all-atom":
                ...
                motif_array.append(
                    array[(array.chain_id == chain_id) & (array.hetero == False) & (array.atom_name == "CA")])
            elif atom_part == "backbone":
                motif_array.append(array[(array.chain_id == chain_id) & (array.hetero == False) & (
                            (array.atom_name == "N") | (array.atom_name == "CA") | (array.atom_name == "C") | (
                                array.atom_name == "O"))])
    else:
        for i in position:
            chain_id = i[0]
            ...
            if atom_part == "all-atom":
                motif_array.append(array[(array.chain_id == chain_id) & (array.res_id <= end) & (
                            array.res_id >= start) & (array.hetero == False)])
            elif atom_part == "CA":
                motif_array.append(array[(array.chain_id == chain_id) & (array.res_id <= end) & (
                            array.res_id >= start) & (array.hetero == False) & (array.atom_name == "CA")])
            elif atom_part == "backbone":
                motif_array.append(array[(array.chain_id == chain_id) & (array.res_id <= end) & (
                            array.res_id >= start) & (array.hetero == False) & (
                                                     (array.atom_name == "N") | (array.atom_name == "CA") | (
                                                         array.atom_name == "C") | (array.atom_name == "O"))])
...
class SingleMotifFactory:
    ...
    def create_batch_motif(self, batch, zeroes=False):
        ...
        motif_n_seg_batch = (
                ...
                np.minimum(motif_n_res_batch,
                           np.full_like(motif_n_res_batch, self.motif_max_n_seg)) - self.motif_min_n_seg + 1
        ...
        motif_sequence_masks = torch.nn.utils.rnn.pad_sequence(motif_sequence_masks, batch_first=True,
                                                               padding_value=False)
        ...
        result['x_motif'] = (result['x_motif'] - mean_w_mask(result['x_motif'], result['fixed_sequence_mask'],
                                                             keepdim=True)) * result['fixed_sequence_mask'][..., None]
